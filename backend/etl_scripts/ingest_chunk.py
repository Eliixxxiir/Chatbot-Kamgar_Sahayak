...
def ingest_file_to_mongodb(file_path: str):
    """
    Reads a JSON file, creates a collection based on its name, 
    and ingests the bilingual data into MongoDB with embeddings for both languages.
    """
    if not sbert_model:
    ...
    try:
        ...
        for chunk in chunks:
            if "content_en" in chunk and "content_hi" in chunk:
                # Generate embeddings for both English and Hindi content
                try:
                    ...
                    # Using update_one with upsert=True to prevent duplicates
...
def main():
    """Main function to find all JSON files and start ingestion."""
